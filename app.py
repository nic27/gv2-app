...
if menu == "📝 Encodage":
    ...
    if not collabs or not clients:
        st.warning("⚠️ Pour encoder, vous devez d'abord avoir des Clients et Collaborateurs. Importez un CSV ou ajoutez-les manuellement dans **Paramètres**.")
    else:
        with st.form("form_encodage", clear_on_submit=True):
            ...
            if st.form_submit_button("🚀 ENREGISTRER LA PRESTATION", use_container_width=True):
                if t > 0:
                    ...
                    conn.commit()
                    st.success(f"Enregistré : {t}h pour {cli} par {col}")
                    st.balloons()
                else: st.error("Le temps doit être supérieur à 0.")

# --- 2. DASHBOARD (FILTRES COMPLETS) ---
elif menu == "📊 Dashboard":
    st.header("📊 Dashboard")
    ...
    if not df.empty:
        ...
        if not df_f.empty:
            ...
            c1, c2 = st.columns(2)
            c1.plotly_chart(px.pie(df_f, values='fact_client', names='client', title="Répartition CA par Client", color='client', color_discrete_map=get_dynamic_colors()), use_container_width=True)
            c2.plotly_chart(px.bar(df_f.groupby('collab')['temps'].sum().reset_index(), x='collab', y='temps', title="Heures par Collab", color='collab', color_discrete_map=get_dynamic_colors()), use_container_width=True)
    else: st.info("Base vide.")

# --- 4. PARAMÈTRES (MAINTENANCE + SYNC) ---
        ...
        with c1:
            ...
            if os.path.exists(DB_PATH):
                with open(DB_PATH, "rb") as f:
                    st.download_button(f"📥 Télécharger la base actuelle", f, f"backup_gv2_{DATE_FILE}.db")
        with c2:
            st.subheader("Restauration")
            up_db = st.file_uploader("Fichier .db", type="db")
            if up_db and st.button("🔥 Écraser tout et Restaurer"):
                with open(DB_PATH, "wb") as f: f.write(up_db.getbuffer())
                st.success("Restauration OK !"); st.rerun()
