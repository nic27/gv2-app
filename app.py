...
# CONFIGURATION MYSQL - REMPLACEZ PAR VOS ACCÈS CLOUD
# Si vous utilisez Streamlit Cloud, il est conseillé d'utiliser st.secrets pour ces infos
DB_CONFIG = {
    "host": "VOTRE_HOST",
    "user": "VOTRE_USER",
...
# --- ONGLET 2 : DASHBOARD ---
    ...
    if not df.empty:
        ...
        recap.columns = ['Société', 'Collaborateur', 'Total Heures', 'Taux Horaire Moyen', 'Total (€)']
        st.dataframe(recap, use_container_width=True, hide_index=True)

        # Graphiques
        c1, c2 = st.columns(2)
        c1.plotly_chart(px.bar(df_f, x='client', y='fact_client', color='client', color_discrete_map=cmap, title="CA par Client"), use_container_width=True)
        c2.plotly_chart(px.pie(df_f, values='temps', names='collab', color='collab', color_discrete_map=cmap, title="Répartition Heures"), use_container_width=True)

        # EXPORTATION DES FILTRES (Réintégré)
        csv = df_f.to_csv(index=False, sep=';', encoding='utf-8-sig').encode('utf-8-sig')
        st.download_button("📥 Exporter la sélection (CSV)", csv, "export_gv2.csv", "text/csv", use_container_width=True)
    else: st.info("Aucune donnée disponible.")

# --- ONGLET 3 : GESTION ---
elif menu == "🛠️ Gestion":
    st.header("🛠️ Édition des Prestations")
...
# --- ONGLET 4 : PARAMÈTRES ---
    ...
    # Section pour ajouter/supprimer Collabs et Clients (Simplifié pour le code)
    st.info("Utilisez cet espace pour configurer vos listes et couleurs.")
...
# --- ONGLET 5 : INFO (Réintégré) ---
    ...
    * **Dashboard** : Utilisez les filtres à gauche pour isoler un collaborateur ou un client.
    * **Export** : Le bouton d'exportation dans le Dashboard génère un CSV basé sur vos filtres.
    * **Sécurité** : Les suppressions sont définitives après confirmation.
    * **Base de données** : MySQL Cloud (Accès multi-utilisateurs simultanés).
    """)
